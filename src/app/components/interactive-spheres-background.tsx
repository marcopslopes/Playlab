...
// Cognitive Calm color palette - soft, therapeutic colors
const calmColors = [
  '#7D9D9C', // sage
  '#C08B7E', // clay
  '#7C8B95', // slate
...
interface InteractiveSpheresBackgroundProps {
  count?: number;
  followCursor?: boolean;
}

export function InteractiveSpheresBackground({ 
  count = 60,
  followCursor = true 
}: InteractiveSpheresBackgroundProps) {
  ...
  useEffect(() => {
    ...
    // Cognitive Calm configuration - gentle, slow, therapeutic
    const config = {
      count,
      materialParams: { 
        metalness: 0.4, 
      ...
      maxSize: 1.2, 
      size0: 1.5,
      gravity: 0.15,  // Very gentle gravity
      friction: 0.985,  // High friction for slow, calm movement
      wallBounce: 0.15, 
      maxVelocity: 0.08,  // Slow max velocity
      maxX: 10, 
      maxY: 10, 
      maxZ: 10,
      controlSphere0: true, 
  ...
  return (
    <canvas 
      ...
      style={{ 
        pointerEvents: 'none',
        opacity: 0.7,  // Subtle, not overwhelming
      }}
    />
  );
}
